...
class ModelConfig:
    ...
    def __init__(
        ...
        has_interleaved_attention = (sliding_window is not None) and (
            ...
            or (self.hf_text_config.model_type in ["gemma2"])
        )
    ...
    def get_num_attention_layers(self, parallel_config: "ParallelConfig") -> int:
        ...
        layers = getattr(
            self.hf_config, "layers_block_type", ["attention"] * num_layers
        )
...
class ParallelConfig:
    ...
    def __init__(
        ...
        if worker_use_ray:
            if self.distributed_executor_backend is None:
                ...
                raise ValueError(
                    f"worker-use-ray can't be used with "
    ...
    def _verify_args(self) -> None:
        ...
        if self.distributed_executor_backend not in ("ray", "mp", None) and not (
            isinstance(self.distributed_executor_backend, type)
...
class PromptAdapterConfig:
    ...
    def __post_init__(self):
        ...
        if self.max_prompt_adapters < 1:
            raise ValueError(
                f"max_prompt_adapters " f"({self.max_prompt_adapters}) must be >= 1."
            )
...
class ObservabilityConfig:
    ...
    def __post_init__(self):
        ...
        if (
            self.collect_model_forward_time or self.collect_model_execute_time
