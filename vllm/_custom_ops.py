...
if hasattr(torch.ops._C, "gptq_marlin_24_gemm"):
    ...
    def _ggml_dequantize_fake(
        W: torch.Tensor, quant_type: int, m: torch.SymInt, n: torch.SymInt
    ) -> torch.Tensor:
    ...
    def machete_prepack_B_fake(
        b_q_weight: torch.Tensor, b_type: ScalarType
    ) -> torch.Tensor:
...
def machete_gemm(
    ...
    return torch.ops._C.machete_gemm(
        a, b_q, b_type.id, b_scales, b_zeros, b_group_size, c, alpha, beta, schedule
    )


def machete_prepack_B(b_q_weight: torch.Tensor, b_type: ScalarType) -> torch.Tensor:
...
def scaled_fp8_quant(
    ...
    out_dtype: torch.dtype = (
        torch.float8_e4m3fnuz if vllm.utils.is_hip() else torch.float8_e4m3fn
    )
...
# gguf
def ggml_dequantize(W: torch.Tensor, quant_type: int, m: int, n: int) -> torch.Tensor:
