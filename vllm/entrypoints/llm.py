...
class LLM:
    ...
    def _add_guided_params(
        ...
        if params.guided_decoding is not None:
            raise ValueError(
                "Cannot set both guided_options_request and" "params.guided_decoding."
            )
    ...
    def _run_engine(
        ...
        total_out_toks = 0
        curr_iter = 0
        MProfile().cuda_profiler_start()

        while self.llm_engine.has_unfinished_requests():
            rng = MProfile().nvtx_start_range(f"iter {curr_iter}", "green")
            ...
            for output in step_outputs:
                if output.finished:
                    ...
                    if use_tqdm:
                        ...
                        pbar.update(1)
            MProfile().nvtx_end_range(rng)
            curr_iter += 1

        MProfile().cuda_profiler_stop()
