...
class VisionTransformer(nn.Module):
    ...
    def forward(
        ...
        if USE_XFORMERS_OPS:
            ...
            raise ImportError(
                "Xformers is required for Pixtral inference " "with the Mistral format"
            )
...
def get_pixtral_hf_patch_grid_length(*, image_size: int, patch_size: int) -> int:
...
def get_max_pixtral_hf_image_feature_size(hf_config: PixtralVisionConfig) -> int:
    return get_pixtral_hf_num_patches(
...
def get_pixtral_hf_image_feature_size(
    hf_config: PixtralVisionConfig, image_width: int, image_height: int
) -> Tuple[int, int]:
...
def input_processor_for_pixtral_hf(
    ...
    *,
    ...
    assert (
        image_feature_size_override is None
    ...
    if new_prompt:
        ...
        for image, next_part in zip(image_data, parts[1:]):
            ...
            replace_tokens = [image_token] * num_width_tokens + [image_break_token]
    ...
    placeholder_indices = [
        idx for idx, token_id in enumerate(new_token_ids) if token_id == image_token_id
    ]
    ...
    for placeholder_idx, replace_tokens in zip(
        reversed(placeholder_indices), reversed(replace_tokens_list)
    ):
...
class PixtralHFAttention(nn.Module):
    ...
    def forward(
        ...
        if USE_XFORMERS_OPS:
            ...
            v = v.reshape(batch, patches, self.n_heads, self.head_dim).transpose(1, 2)
