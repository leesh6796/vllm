...
from vllm.model_executor.layers.quantization import AWQConfig, QuantizationConfig
...
from .clip import dummy_image_for_clip, dummy_seq_data_for_clip, get_clip_num_patches
...
def calculate_num_blocks_wrapper(
    hf_config: PretrainedConfig, max_dynamic_patch: Optional[int] = None
):
...
def dynamic_preprocess(
    image: Image.Image, min_num: int, max_num: int, image_size: int, use_thumbnail: bool
) -> List[Image.Image]:
...
def image_to_pixel_values(
    image: Image.Image, input_size: int, min_num: int, max_num: int, use_thumbnail: bool
) -> torch.Tensor:
...
def get_max_internvl_image_tokens(
    ctx: InputContext, *, max_dynamic_patch: Optional[int] = None
):
...
class InternVLInputPipeline:
    ...
    def input_processor(
        ...
        *,
        ...
        new_prompt = self._expand_image_prompt(prompt, image_feature_sizes, num_patches)
    ...
    def input_mapper(
        ...
        *,
        ...
        image_token_id = tokenizer.encode(
            self.img_context_token, add_special_tokens=False, return_tensors="pt"
        )[0]

        return MultiModalInputs(
            {"pixel_values": data, "image_token_id": image_token_id}
        )
