...
from transformers import CLIPVisionConfig, LlavaOnevisionConfig, SiglipVisionConfig
...
def get_llava_onevision_image_feature_size(
    ...
    *,
    ...
    (
        ...
        input_height, input_width, num_patches, num_patch_height, num_patch_width
    )
...
def input_processor_for_llava_onevision(ctx: InputContext, inputs: DecoderOnlyInputs):
    multi_modal_data = inputs.get("multi_modal_data")
...
class LlavaOnevisionForConditionalGeneration(nn.Module, SupportsMultiModal, SupportsPP):
    ...
    def _video_pixels_to_features(
        ...
        video_features = video_features.reshape(b, frames * video_features.shape[1], -1)
        image_newline = (
            self.image_newline[None, None, :].repeat(b, 1, 1).to(video_features.device)
        )
