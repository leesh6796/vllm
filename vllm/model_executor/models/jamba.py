...
from vllm.worker.model_runner import _BATCH_SIZES_TO_CAPTURE, _get_graph_batch_size
    ...
    **selective** state spaces)
    ...
    def __init__(self, config: JambaConfig):
        ...
        self.dt_proj = ColumnParallelLinear(
            self.time_step_rank, self.intermediate_size, bias=True, skip_bias_add=True
        )
...
class JambaMambaDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        hidden_states = self.mamba(hidden_states, attn_metadata, mamba_cache_params)
        # Fully Connected
    ...
    def forward(
        ...
        for i in range(len(self.layers)):
            ...
            if isinstance(layer, JambaAttentionDecoderLayer):
                kv_cache = kv_caches[
                    (i - self.config.attn_layer_offset) // self.config.attn_layer_period
                ]
            if isinstance(layer, JambaMambaDecoderLayer):
                current_state_layer = i - (
                    1
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.mamba_cache is None:
            max_batch_size = (
                _get_graph_batch_size(self.scheduler_config.max_num_seqs)
        ...
        ) = self.mamba_cache.current_run_tensors(input_ids, attn_metadata, **kwargs)
        mamba_cache_params = MambaCacheParams(
            mamba_cache_tensors[0], mamba_cache_tensors[1], state_indices_tensor
        )
        hidden_states = self.model(
            input_ids, positions, kv_caches, attn_metadata, mamba_cache_params
        )
        return hidden_states
