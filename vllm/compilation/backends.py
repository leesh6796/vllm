...
def vllm_backend(
    graph, example_inputs, additional_inductor_config: Optional[Dict] = None
) -> Callable:
    ...
    def compiled_graph_wrapper(*args):
        ...
        runtime_shapes: Tuple[int, ...] = tuple(args[i] for i in sym_shape_indices)
        ...
        if runtime_shapes not in runtime_shapes_to_compile_flags:
            ...
            runtime_shapes_to_compile_flags[runtime_shapes] = (
                runtime_shapes[0] in sizes_to_specialize
            )
...
def select_default_backend(level: int) -> Union[str, Callable]:
    ...
    if level == CompilationLevel.INDUCTOR_MAX_AUTOTUNE:
        if (
            "max_autotune" in additional_configs
            and not additional_configs["max_autotune"]
        ):
            logger.warning(
    ...
    backend = partial(vllm_backend, additional_inductor_config=additional_configs)
