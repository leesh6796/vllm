...
environment_variables: Dict[str, Callable[[], Any]] = {
    ...
    # on each node.
    "VLLM_HOST_IP": lambda: os.getenv("VLLM_HOST_IP", "") or os.getenv("HOST_IP", ""),
    ...
    "VLLM_TORCH_COMPILE_LEVEL": lambda: int(
    ...
    "VLLM_CUSTOM_OPS": lambda: os.environ.get("VLLM_CUSTOM_OPS", "")
    .replace(" ", "")
    .split(","),
    ...
    "VLLM_USE_FLASHINFER_SAMPLER": lambda: bool(
        int(os.getenv("VLLM_USE_FLASHINFER_SAMPLER", "0"))
    ),
    ...
    "VLLM_CPU_OMP_THREADS_BIND": lambda: os.getenv("VLLM_CPU_OMP_THREADS_BIND", "all"),
    # OpenVINO device selection
    # default is CPU
    "VLLM_OPENVINO_DEVICE": lambda: os.getenv("VLLM_OPENVINO_DEVICE", "CPU").upper(),
    # OpenVINO key-value cache space
    # default is 4GB
    "VLLM_OPENVINO_KVCACHE_SPACE": lambda: int(
        os.getenv("VLLM_OPENVINO_KVCACHE_SPACE", "0")
    ),
    # OpenVINO KV cache precision
    # default is bf16 if natively supported by platform, otherwise f16
    # To enable KV cache compression, please, explicitly specify u8
    "VLLM_OPENVINO_CPU_KV_CACHE_PRECISION": lambda: os.getenv(
        "VLLM_OPENVINO_CPU_KV_CACHE_PRECISION", None
    ),
    # Enables weights compression during model export via HF Optimum
    # default is False
    "VLLM_OPENVINO_ENABLE_QUANTIZED_WEIGHTS": lambda: bool(
        os.getenv("VLLM_OPENVINO_ENABLE_QUANTIZED_WEIGHTS", False)
    ),
    ...
    "VLLM_USE_RAY_COMPILED_DAG_NCCL_CHANNEL": lambda: bool(
        int(os.getenv("VLLM_USE_RAY_COMPILED_DAG_NCCL_CHANNEL", "1"))
    ),
    ...
    # Default is 5 seconds
    "VLLM_AUDIO_FETCH_TIMEOUT": lambda: int(os.getenv("VLLM_AUDIO_FETCH_TIMEOUT", "5")),
    ...
    "VLLM_NO_DEPRECATION_WARNING": lambda: bool(
        int(os.getenv("VLLM_NO_DEPRECATION_WARNING", "0"))
    ),
    ...
    "VLLM_SKIP_P2P_CHECK": lambda: os.getenv("VLLM_SKIP_P2P_CHECK", "0") == "1",
    ...
    "VLLM_USE_V1": lambda: bool(int(os.getenv("VLLM_USE_V1", "0"))),
}
